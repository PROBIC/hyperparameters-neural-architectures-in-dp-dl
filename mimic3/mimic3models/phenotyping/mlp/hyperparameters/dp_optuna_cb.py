...
def train_with_params(
    ...
    if cached_data_path is None:
        args["data"] = os.path.join(
            os.path.dirname(__file__), "../../../data/phenotyping/"
        )
    else:
    ...
    for epoch in range(dp_params.get("num_epochs")):
        ...
        stats_dict["title"] = "mimic3_phenotyping_mlp_dp_width"
...
def return_objective(paramvalue):
    def objective(trial):
        ...
        num_epochs = trial.suggest_int("num_epochs", 1, 300) # was 200
        batch_size = trial.suggest_int("batch_size", 32, 3072) # was 2048
        n_layers = 2
        out_dims_list = list()
        out_dims_list.append(81)
        ...
        for seed in seeds:
            auc_dict, epsilon = train_with_params(
                ...
                stats_path="/mimic3experiments/mlp_optuna_cb_new_exp_{}_{}.csv".format(
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="mlp_cb_nm_new_{}_{}".format(str(sys.argv[1]), str(sys.argv[2])),
        direction="maximize",
    )
    ...
    joblib.dump(
        study,
        str(
            "/mimic3experiments/mlp_dp_cb_new_{}_{}.pkl".format(
                str(sys.argv[1]), sys.argv[2]
            )
        ),
    )
    for _ in range(20):
        ...
        joblib.dump(
            study,
            str(
                "/mimic3experiments/mlp_cb_new_{}_{}.pkl".format(
                    str(sys.argv[1]), sys.argv[2]
                )
            ),
        )
