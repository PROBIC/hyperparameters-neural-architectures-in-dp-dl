...
def train_with_params(
    ...
    for epoch in range(dp_params.get("num_epochs")):
        ...
        stats_dict["title"] = "mimic3_phenotyping_mlp_width_bn_v2"
    ...
    def objective(trial):
        ...
        for seed in seeds:
            auc_dict = train_with_params(
                ...
                stats_path="/mimic3experiments/mlp_non_dp_optuna_architecture_width_batchnorm_v2_exp_{}.csv".format(
                    str(sys.argv[1])
                ),
                seed=seed,
                train_X=train_X,
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="mlp_non_dp_width_batchnorm_v2_{}".format(str(sys.argv[1])),
        direction="maximize",
    )
    ...
    joblib.dump(
        study,
        str(
            "/mimic3experiments/mlp_non_dp_width_batchnorm_v2_{}.pkl".format(
                str(sys.argv[1])
            )
        ),
    )
