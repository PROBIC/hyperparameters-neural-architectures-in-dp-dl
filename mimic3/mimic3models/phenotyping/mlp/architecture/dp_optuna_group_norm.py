...
def train_with_params(
    ...
    for epoch in range(dp_params.get("num_epochs")):
        ...
        stats_dict["num_params"] = sum(p.numel() for p in model.parameters() if p.requires_grad)
        columns = [
            "title",
            "ave_auc_micro",
            "ave_auc_macro",
    ...
    class MLPMultiLabel(torch.nn.Module):
        def __init__(self, input_dim, output_dim, n_layers, out_dims_list):
            ...
            for i in range(n_layers - 1):
                ...
                if dp_params.get("n_groups") != -1:
                    layers.append(torch.nn.GroupNorm(dp_params.get("n_groups"), out_dims_list[i]))
...
def return_objective(paramvalue):
    def objective(trial):
        ...
        out_dims_list = list()
        for i in range(n_layers - 1):
            out_dims_list.append(120)
        seeds = [374148, 521365] #472368
        scores = list()
        ...
        for seed in seeds:
            auc_dict, epsilon = train_with_params(
                ...
                stats_path="/mimic3experiments/mlp_optuna_architecture_group_norm_exp_{}_bf_l{}.csv".format(str(sys.argv[1]), str(sys.argv[2])),
                seed=seed,
            )
...
if __name__ == "__main__":
    study = optuna.create_study(study_name="mlp_groups_{}".format(str(sys.argv[1])), direction="maximize")
    objective = return_objective(None)
    joblib.dump(study, str("/mimic3experiments/mlp_groups_{}_bf_l{}.pkl".format(str(sys.argv[1]), str(sys.argv[2]))))
    for _ in range(30):
