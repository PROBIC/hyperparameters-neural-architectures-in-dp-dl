...
def train_with_params(
    ...
    elapsed_time = copy(start.elapsed_time(end))
    ...
    stats_dict["elapsed_training_time"] = elapsed_time
...
if __name__ == "__main__":
    csv_path = "/mimic3/mimic3models/phenotyping/mlp/best_for_time_measurements/bs.csv"
    ...
    all_runs = np.ravel([list_of_dicts] * 4)
    np.random.shuffle(all_runs)

    for i, c in enumerate(all_runs):
        print("Looking at config {}/{}".format(i, len(all_runs)), flush=True)
        auc_dict, epsilon, model = train_with_params(
            c,
            stats_path="/mimic3experiments/mlp_dp_bs_time_measurement_2.csv",
            seed=int(c["seed"])
        )
