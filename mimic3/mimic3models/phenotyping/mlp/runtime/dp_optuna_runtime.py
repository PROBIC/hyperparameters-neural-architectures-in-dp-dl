...
def train_with_params(
    ...
    for epoch in range(dp_params.get("num_epochs")):
        ...
        stats_dict["title"] = "mimic3_phenotyping_mlp_dp_runtime"
        ...
        time_diff = time.time() - start_model
        if time_diff > float(sys.argv[1]):
            break

        stats_dict["time_diff"] = time_diff
    ...
    def objective(trial):
        ...
        for seed in seeds:
            auc_dict, epsilon = train_with_params(
                ...
                stats_path="/mimic3experiments/mlp_optuna_architecture_width_runtime_exp_{}_{}.csv".format(
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="runtime_exp_{}_{}".format(str(sys.argv[1]), str(sys.argv[2])),
        direction="maximize",
    )
    ...
    joblib.dump(
        study,
        str(
            "/mimic3experiments/mlp_optuna_architecture_width_runtime_exp_{}_{}.pkl".format(
                str(sys.argv[1]), sys.argv[2]
            )
        ),
    )
